		...
		try {
			if ("+".equals(operandsDTO.getOperationType())) {
				operandsDTO.setResult(
						calculatorService.sum(operandsDTO.getFirstOperand(), operandsDTO.getSecondOperand()));
				...
				operandsDTO.setResult(
						calculatorService.divide(operandsDTO.getFirstOperand(), operandsDTO.getSecondOperand()));
			} else {
